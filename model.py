...
def p_bad_mc(rng, n, mode, w=W, budget=BUDGET, trials=TRIALS, rho=0.0):
    ...
    if mode == "parallel" and rho > 0:
        ...
        # fan-out at once, so the aggregate is one common slow draw, not the max.
        # It fattens the tail past the independent analytic (which is why rho=0
        # is the optimistic floor).
        corr = rng.random(trials) < rho
        shared = rng.normal(SLOW_MEAN, SLOW_SD, trials)
        agg = np.where(corr, shared, agg)
